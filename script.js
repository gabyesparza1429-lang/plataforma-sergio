...
function renderReminders() {
    ...
    // Filtrar: solo mostrar los que no han pasado de fecha
    ...
    activeReminders.forEach((r, index) => {
...
// --- FUNCIONES ADMIN ---
function openAdmin() {
    // Intentar abrir con prompt
    const p = window.prompt("Mot de passe :");
    if (p === "Gaby1429") {
        showView('view-admin');
    } else if (p !== null) {
...
function saveReminder() {
    ...
    if(!text || !date) return alert("Remplis le rappel et la date !");
...
function saveFinalScore() {
    ...
    reports.push({ materia: currentSub, note: note, date: new Date().toLocaleString() });
